...
namespace Masters
{
	namespace Matrix
	{
		...
		bool Location::valid(const CLocation& loc) noexcept
		{
			return ((loc.ordinate() >= COrdinate(0)) && (loc.abscissa() >= CAbscissa(0)));
		}

		bool Location::valid(const CLocation& loc,
			const Size::CHeight& height, const Size::CLength& length) noexcept
		{
			return (valid(loc) && ((loc.ordinate() < (int)(height)) && (loc.abscissa() < (int)(length))));
		}

		bool Location::valid(const CLocation& loc, const CSize& sizes) noexcept
		{
			return (valid(loc) && ((loc.ordinate() < (int)(sizes.height())) && (loc.abscissa() < (int)(sizes.height()))));
		}
	}
}
